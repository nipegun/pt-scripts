...
# Escaneo de registros de cada Slave ID válido
  for vSlaveID in "${aSlaveIDsValidos[@]}"; do
    ...
    echo ""
    timeout "$vTimeOut" mbpoll -m tcp -t 3 -a $vSlaveID -r 0 -c 125 -0 -1 -q "$vIP" || echo "    ⚠️ Fallo al leer Input Registers del Slave ID $vSlaveID"

    echo ""
    echo "  🔍 Escaneando Holding Registers (0x03) del Slave ID $vSlaveID..."
    echo ""
    timeout "$vTimeOut" mbpoll -m tcp -t 4 -a $vSlaveID -r 0 -c 125 -0 -1 -q "$vIP" || echo "    ⚠️ Fallo al leer Holding Registers del Slave ID $vSlaveID"
