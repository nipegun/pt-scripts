...
# Escanear el PLC en búsqueda de Slave IDs válidos
  ...
  for vSlaveID in $(seq 1 3); do
    ...
    mbpoll -m tcp -t 4 -a $vSlaveID -r 0 -c 1 -0 -1 -o 1 $vIP > /dev/null 2>&1
  if [ $? -eq 0 ]; then
    echo "✅ Responde"
...
# Escaneo de registros de cada Slave ID válido
  ...
  for vSlaveID in "${aSlaveIDsValidos[@]}"; do
    ...
    echo ""
    timeout "$vTimeOut" mbpoll -m tcp -t 0 -a $slaveID -r 0 -c 2000 -0 -1 "$vIP" || echo "    ⚠️ Fallo al leer Coils del Slave ID $vSlaveID"

    echo ""
    echo "🔍 Escaneando Discrete Inputs (0x02) del Slave ID $vSlaveID..."
    echo ""
    timeout "$vTimeOut" mbpoll -m tcp -t 1 -a $slaveID -r 0 -c 2000 -0 -1 "$vIP" || echo "    ⚠️ Fallo al leer Inputs del Slave ID $vSlaveID"

    echo ""
    echo "🔍 Escaneando Input Registers (0x04) del Slave ID $vSlaveID..."
    echo ""
    timeout "$vTimeOut" mbpoll -m tcp -t 3 -a $slaveID -r 0 -c 125 -0 -1 "$vIP" || echo "    ⚠️ Fallo al leer Input Registers del Slave ID $vSlaveID"

    echo ""
    echo "🔍 Escaneando Holding Registers (0x03) del Slave ID $vSlaveID..."
    echo ""
    timeout "$vTimeOut" mbpoll -m tcp -t 4 -a $slaveID -r 0 -c 125 -0 -1 "$vIP" || echo "    ⚠️ Fallo al leer Holding Registers del Slave ID $vSlaveID"
